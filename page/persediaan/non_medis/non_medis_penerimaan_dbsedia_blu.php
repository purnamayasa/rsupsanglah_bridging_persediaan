...
$tabel = 'non_medis_penerimaan_dbsedia_blu_excel';
?>
<h3>Import Excel Non Medis Persediaan BLU</h3>
	...
	while ($row = $rs->fetch_object()) {
		...
		if ($_POST['submit'] == 'Compare') {
			$sqlc = "
				...
				FROM 	
					`rsupsanglah`.`non_medis_stokopname_excel`
				WHERE 
					`tgl` = '{$_POST['tanggal_compare']}'
				AND `kode_barang` = '{$row->kd_brg2}'
			";	
